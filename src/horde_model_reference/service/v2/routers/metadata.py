...
route_registry.register_route(
    v2_prefix + "/metadata",
    RouteNames.get_v2_last_updated,
...
@router.get(
    ...
    summary="Get last update timestamp for canonical format (v2)",
    operation_id="read_v2_last_updated",
    responses={
        200: {
            "description": "Last updated timestamp",
            "links": {
                "GetAllV2Metadata": {
                    "operationId": "read_all_v2_metadata",
                    "description": "Get all v2 format metadata for all categories.",
                },
            },
        },
...
async def read_v2_last_updated(
    ...
    for v2 format operations. Only available when canonical_format='v2'.
    ...
    # Get all v2 metadata and find max timestamp
    all_metadata = manager.backend.get_all_metadata()
...
route_registry.register_route(
    v2_prefix + "/metadata",
    RouteNames.get_v2_category_last_updated,
...
@router.get(
    ...
    summary="Get last update timestamp for a specific category (v2)",
    operation_id="read_v2_category_last_updated",
    responses={
        200: {
            "description": "Category last updated timestamp",
            "links": {
                "GetCategoryMetadata": {
                    "operationId": "read_v2_category_metadata",
                    "parameters": {
...
async def read_v2_category_last_updated(
    ...
    metadata = manager.backend.get_metadata(model_category_name)

    if metadata is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No v2 metadata found for category {model_category_name.value}",
        )
...
route_registry.register_route(
    v2_prefix + "/metadata",
    RouteNames.get_all_v2_metadata,
...
@router.get(
    ...
    summary="Get all v2 format metadata",
    operation_id="read_all_v2_metadata",
    responses={
        200: {
            "description": "All v2 format metadata",
            "links": {
                "GetCategoryMetadata": {
                    "operationId": "read_v2_category_metadata",
                ...
                "GetLastUpdated": {
                    "operationId": "read_v2_last_updated",
...
async def read_all_v2_metadata(
    manager: Annotated[ModelReferenceManager, Depends(get_model_reference_manager)],
    ...
    return manager.backend.get_all_metadata()
...
route_registry.register_route(
    v2_prefix + "/metadata",
    RouteNames.get_v2_category_metadata,
...
@router.get(
    ...
    summary="Get v2 format metadata for a specific category",
    operation_id="read_v2_category_metadata",
    responses={
        200: {
            "description": "Category metadata",
            "links": {
                "GetCategoryLastUpdated": {
                    "operationId": "read_v2_category_last_updated",
                    "parameters": {
                        "model_category_name": "$request.path.model_category_name",
                    },
...
async def read_v2_category_metadata(
    model_category_name: MODEL_REFERENCE_CATEGORY,
    manager: Annotated[ModelReferenceManager, Depends(get_model_reference_manager)],
) -> CategoryMetadata:
